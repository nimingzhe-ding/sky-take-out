...
//        vo.setPackageStr(jsonObject.getString("package"));
    ...
    public void paySuccess(String outTradeNo) {
        ...
        Map map = new HashMap();
        map.put("type",1);//1表示接单提醒，2表示客户催单
        map.put("orderId",ordersDB.getId());
        map.put("content","有新订单待接单，订单号："+outTradeNo);
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        Page<Orders> page = orderMapper.pageQuery(ordersPageQueryDTO);
        //部分订单需要额外返回订单菜品信息，将orders变为ordersVo
...
//将订单所有菜品拼接在一起
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) {
        ...
        if (payStatus == Orders.PAID) {
            ...
            log.info("订单已支付，走退款流程，订单号：{}", ordersDB.getNumber());
        }
    ...
    public void cancel(OrdersCancelDTO ordersCancelDTO) {
        ...
        //管理端取消订单需要进行退款，根据订单id更新订单状态，取消原因，取消时间
        Orders orders = new Orders();
    ...
    public void reminder(Long id) {
        ...
        Map map = new HashMap();
        map.put("type",2);//1表示接单提醒，2表示客户催单
        map.put("orderId",ordersDB.getId());
